...
_period_payload_cache: dict[tuple[bool, str], tuple[float, list[dict]]] = {}
...
def _build_period_placeholder(period_key: str) -> dict:
    label = PERIOD_TABLE_MAP.get(period_key, (period_key, "", ""))[0]
    return {
        "table": period_key,
        "label": label,
        "calculated_at": "-",
...
def _build_period_payload(
    ...
    for period_key, _, _, _ in PERIODS:
        ...
        if include_placeholders:
            payload.append(_build_period_placeholder(period_key))
...
def _fetch_channel_growth_snapshot(limit: int = 100) -> dict:
        ...
        channels.append(
            {
                ...
                "top_video_ids": [
                    str(v).strip()
                    for v in str(row.get("top_video_ids_csv") or "").split(",")
                    if str(v).strip()
                ],
            }
        )
    ...
    for item in channels:
        for video_id in item.get("top_video_ids") or []:
...
def _fetch_channel_growth_sidebar_metrics() -> dict:
    metrics = {
        "total_likes": 0,
        "total_comments": 0,
        "likes_24h": 0,
    ...
    try:
        rows = fetchall(
            """
            SELECT
                COALESCE(SUM(x.like_count), 0) AS total_likes,
                COALESCE(SUM(x.comment_count), 0) AS total_comments
            FROM (
                SELECT DISTINCT ON (video_id)
                    video_id,
                    like_count,
        ...
        if rows:
            metrics["total_likes"] = int(rows[0].get("total_likes") or 0)
            metrics["total_comments"] = int(rows[0].get("total_comments") or 0)
    except Exception:
        logger.exception("Failed to fetch channel-growth sidebar metrics")
    try:
        rows = fetchall(
            """
            WITH latest_per_video AS (
            ...
            calc AS (
                SELECT
                    ...
                    GREATEST(
                        0,
                        COALESCE(s_latest.like_count, 0) - COALESCE(
                            (
                                SELECT s_prev.like_count
                                FROM video_stats s_prev
                                WHERE s_prev.video_id = l.video_id
                                  AND s_prev.timestamp <= (l.latest_ts - INTERVAL '24 hours')
                                ORDER BY s_prev.timestamp DESC
                                LIMIT 1
                            ),
                            COALESCE(s_latest.like_count, 0)
                        )
                    ) AS like_growth_24h,
                    GREATEST(
                        0,
                        COALESCE(s_latest.comment_count, 0) - COALESCE(
                            (
                                SELECT s_prev.comment_count
                                FROM video_stats s_prev
                                WHERE s_prev.video_id = l.video_id
                                  AND s_prev.timestamp <= (l.latest_ts - INTERVAL '24 hours')
                                ORDER BY s_prev.timestamp DESC
                            ...
                            COALESCE(s_latest.comment_count, 0)
                        )
                    ) AS comment_growth_24h
                FROM latest_per_video l
                JOIN video_stats s_latest
                  ON s_latest.video_id = l.video_id
                 AND s_latest.timestamp = l.latest_ts
            )
            SELECT
                COALESCE(SUM(like_growth_24h), 0) AS likes_24h,
                COALESCE(SUM(comment_growth_24h), 0) AS comments_24h
            FROM calc
            """
        )
        if rows:
            metrics["likes_24h"] = int(rows[0].get("likes_24h") or 0)
            metrics["comments_24h"] = int(rows[0].get("comments_24h") or 0)
    except Exception:
        logger.exception("Failed to fetch channel-growth 24h like/comment metrics")
    try:
        rows = fetchall(
            """
            SELECT COALESCE(SUM(x.view_count), 0) AS total_views
            FROM (
                SELECT DISTINCT ON (video_id)
                    video_id,
                    view_count
        ...
        if rows:
            metrics["total_views"] = int(rows[0].get("total_views") or 0)
    except Exception:
        logger.exception("Failed to fetch channel-growth total views")
    return metrics


def render_channel_growth_page(base_url: str = "") -> str:
    data = _fetch_channel_growth_snapshot(limit=100)
    new_pickups = _fetch_channel_growth_new_pickups(limit=5)
    hero_stats = _fetch_public_hero_stats()
    side_metrics = _fetch_channel_growth_sidebar_metrics()
    updated_at = _fmt_datetime(data.get("updated_at"))
    updated_date = updated_at.split(" ")[0] if updated_at and updated_at != "-" else "-"
    growth_ranked = data.get("growth_ranked") or []
    rate_ranked = data.get("rate_ranked") or []
    video_meta = data.get("video_meta") or {}

    def _avatar_html(item: dict) -> str:
        icon_url = html.escape(str(item.get("channel_icon_url") or ""), quote=True)
        label = html.escape((str(item.get("channel_name") or "CH")[:1] or "CH").upper())
        if icon_url:
            return (
                f'<img src="{icon_url}" alt="" loading="lazy" '
                "onerror=\"this.style.display='none';const fb=this.parentElement?.querySelector('.avatar-fallback');if(fb) fb.style.display='inline-flex';\">"
                f'<span class="avatar-fallback" style="display:none;">{label}</span>'
            )
        return f'<span class="avatar-fallback">{label}</span>'
    ...
    def _top_video_cards_html(item: dict) -> str:
        ...
        for video_id in (item.get("top_video_ids") or [])[:1]:
            ...
            title = html.escape(str(meta.get("title") or "動画詳細"))
            thumb = html.escape(str(meta.get("thumbnail_url") or ""), quote=True)
            thumb_html = (
                f'<img src="{thumb}" alt="" loading="lazy">'
                if thumb
                else '<span class="embedded-video-thumb-fallback">VIDEO</span>'
            )
            cards.append(
                f'<a class="embedded-video-card" href="/video/{quote(vid, safe="")}">'
                f'<span class="embedded-video-thumb">{thumb_html}</span>'
                f'<span class="embedded-video-title">{title}</span></a>'
            )
        return "".join(cards)
    ...
    for idx, item in enumerate(growth_ranked[:50], start=1):
        ...
        embedded_videos = _top_video_cards_html(item)
        avatar = _avatar_html(item)
        growth_value = int(item.get("growth_24h") or 0)
        row_videos = f'<div class="embedded-video-list icon-anchor">{embedded_videos}</div>' if idx <= 10 else ""
        growth_items.append(
            f'<div class="rank-row">'
            f'<span class="rank-badge {cls}">{idx}</span>'
            f'<span class="avatar">{avatar}</span>'
            f'<span class="row-main"><a class="row-title-link" href="{link}" target="_blank" rel="noopener noreferrer">{channel_name}</a>'
            ...
            f'<span class="row-value">+{growth_value:,}</span>{row_videos}</div>'
        )
    rate_items: list[str] = []
    for idx, item in enumerate(rate_ranked[:50], start=1):
        ...
        rate_items.append(
            ...
            f'<span class="row-value rate">+{rate_value:.1f}%</span>{row_videos}</div>'
        )
    ...
    top_growth_name = html.escape(str((growth_ranked[0].get("channel_name") if growth_ranked else "") or "-"))
    top_growth_value = int((growth_ranked[0].get("growth_24h") if growth_ranked else 0) or 0)
    top_rate_name = html.escape(str((rate_ranked[0].get("channel_name") if rate_ranked else "") or "-"))
...
<style>:root{--bg:#fff;--header-bg:rgba(11,17,28,.92);--panel:#fff;--panel-border:#d3dde8;--text-main:#111827;--text-sub:#475569;--text-soft:#64748b;--text-primary:#e8edf4;--ok:#10b981;--header-border:rgba(99,177,230,.22);--shadow-sm:0 8px 18px rgba(15,23,42,.10);--shadow-md:0 14px 30px rgba(15,23,42,.16);--radius-md:12px}*{box-sizing:border-box}body{margin:0;font-family:"Noto Sans JP Local","Hiragino Kaku Gothic ProN",sans-serif;color:var(--text-main);background:radial-gradient(900px 420px at 10% -10%,rgba(99,208,255,.08),transparent 60%),radial-gradient(760px 380px at 92% -12%,rgba(96,165,250,.06),transparent 58%),var(--bg)}.header{position:sticky;top:0;z-index:100;background:var(--header-bg);border-bottom:1px solid rgba(100,160,240,.18);border-top:2px solid rgba(99,208,255,.34)}.header-inner{height:60px;padding:0 24px;display:flex;align-items:center;justify-content:space-between}.brand{display:flex;align-items:center;gap:12px;text-decoration:none;color:var(--text-primary)}.brand-logo{display:inline-flex;align-items:center;justify-content:center;width:44px;height:30px;border-radius:8px;background:linear-gradient(135deg,#63d0ff,#a78bfa);color:#fff;font-weight:900;font-size:.72rem}.brand-text{font-weight:800;font-size:1.05rem}.brand-accent{background:linear-gradient(135deg,#63d0ff,#a78bfa);-webkit-background-clip:text;-webkit-text-fill-color:transparent}.header-meta{color:rgba(232,237,244,.90);font-size:.82rem}.live-dot{display:inline-flex;align-items:center;gap:6px;color:#10b981;font-weight:600}.live-dot::before{content:'';width:7px;height:7px;border-radius:50%;background:#10b981;animation:pulse-dot 2s ease-in-out infinite}@keyframes pulse-dot{0%,100%{opacity:1;transform:scale(1)}50%{opacity:.5;transform:scale(.8)}}.main{max-width:1180px;margin:0 auto;padding:22px 24px 48px;display:grid;gap:14px}.panel{background:var(--panel);border:1px solid var(--panel-border);border-radius:var(--radius-md);box-shadow:var(--shadow-sm);padding:14px}.hero h1{margin:0;font-size:1.1rem;color:#0f294f}.hero p{margin:4px 0 0;color:var(--text-sub);font-size:.86rem}.content{display:grid;grid-template-columns:minmax(0,2fr) minmax(320px,1fr);gap:14px;align-items:start}.tabs{display:inline-flex;border:1px solid var(--panel-border);border-radius:10px;overflow:hidden;margin-top:8px}.tab{border:0;background:transparent;color:#64748b;padding:6px 10px;font-size:.76rem;font-weight:700;cursor:pointer}.tab.active{color:#fff;background:linear-gradient(135deg,rgba(99,208,255,.72),rgba(96,165,250,.82))}.panel-view{display:none}.panel-view.active{display:block}.row-title-link{font-size:.98rem;color:#0f172a;font-weight:700;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;text-decoration:none;display:inline-block}.row-title-link:hover{color:#075985;text-decoration:underline}.embedded-video-list{display:block;margin-top:6px;width:100%}.embedded-video-list.icon-anchor{grid-column:2/4}.embedded-video-card{display:grid;grid-template-columns:68px minmax(0,1fr);gap:8px;align-items:center;width:420px;max-width:100%;margin:0;padding:6px 8px;border:1px solid #b7cce2;border-radius:10px;background:linear-gradient(180deg,#fff,#f8fbff);color:#1f334d;text-decoration:none;box-shadow:0 2px 6px rgba(15,23,42,.08);transition:transform .15s ease,box-shadow .15s ease}.embedded-video-card:hover{transform:translateY(-1px);box-shadow:0 5px 12px rgba(15,23,42,.14)}.embedded-video-thumb{width:68px;height:38px;border-radius:7px;overflow:hidden;background:#111827;display:inline-flex;align-items:center;justify-content:center}.embedded-video-thumb img{width:100%;height:100%;object-fit:cover;display:block}.embedded-video-thumb-fallback{font-size:.56rem;color:#e2e8f0;font-weight:700}.embedded-video-title{font-size:.82rem;line-height:1.3;color:#1f334d;display:-webkit-box;-webkit-box-orient:vertical;-webkit-line-clamp:2;overflow:hidden}.rank-list{display:grid;gap:10px}.rank-row{display:grid;grid-template-columns:42px 42px minmax(0,1fr) auto;gap:10px;align-items:start;border:1px solid var(--panel-border);border-radius:10px;background:#fff;padding:10px}.rank-badge{min-width:34px;height:34px;border-radius:7px;display:inline-flex;align-items:center;justify-content:center;font-size:.9rem;font-weight:800;color:#fff;background:rgba(26,32,44,.82)}.rank-badge.gold{background:linear-gradient(135deg,#f59e0b,#facc15);color:#3b2a00}.rank-badge.silver{background:linear-gradient(135deg,#94a3b8,#e2e8f0);color:#1f2937}.rank-badge.bronze{background:linear-gradient(135deg,#b45309,#d6a77a);color:#fff8ef}.avatar{width:40px;height:40px;border-radius:50%;overflow:hidden;background:#e7eef7;border:1px solid #d6e0ea;display:inline-flex;align-items:center;justify-content:center}.avatar img{width:100%;height:100%;object-fit:cover;display:block}.avatar-fallback{font-size:.72rem;font-weight:800;color:#33516f}.row-main{min-width:0;display:flex;flex-direction:column;align-items:flex-start}.row-meta{color:var(--text-soft);font-size:.82rem}.row-value{text-align:right;font-weight:800;color:var(--ok);font-size:.95rem}.row-value.rate{color:#0369a1}.side-stack{display:grid;gap:12px}.pickup-list{display:grid;gap:8px}.pickup-item{display:grid;grid-template-columns:72px minmax(0,1fr);gap:8px;align-items:center;padding:6px;border:1px solid var(--panel-border);border-radius:8px;background:#fff;text-decoration:none;color:inherit}.pickup-thumb{width:72px;height:40px;border-radius:6px;overflow:hidden;background:#0f172a}.pickup-thumb img{width:100%;height:100%;object-fit:cover;display:block}.pickup-main{min-width:0;display:grid}.pickup-title{font-size:.82rem;color:#0f172a;display:-webkit-box;-webkit-box-orient:vertical;-webkit-line-clamp:2;overflow:hidden}.pickup-meta{font-size:.74rem;color:var(--text-soft)}.pickup-head{margin:0 0 10px;color:#1f3344;font-size:1rem;font-weight:800}.side-block{margin:0}.side-info-head{margin:0 0 8px;color:#1f3344;font-size:1rem;font-weight:800}.side-info{display:grid;gap:7px;margin:0}.side-info-row{display:flex;justify-content:space-between;gap:8px;font-size:.92rem;color:#475569}.side-info-row strong{color:#0f172a}.metric-growth{font-weight:800}.metric-growth.views{color:#10b981}.metric-growth.likes{color:#db2777}.metric-growth.comments{color:#334155}.metric-growth .ico{display:inline-block;margin-right:4px}.footer{width:100%;margin:0;padding:18px 24px 22px;text-align:center;font-size:.76rem;color:#dbeafe;border-top:1px solid var(--header-border);background:var(--header-bg)}.footer-links{display:flex;justify-content:center;gap:14px;margin-bottom:8px;font-size:.82rem}.footer-links a{color:#cfe0ff;text-decoration:none}@media (max-width:980px){.content{grid-template-columns:1fr}}@media (max-width:760px){.header-inner{padding:0 14px;height:54px}.header-meta{display:none}.main{padding:16px 14px 34px}.rank-row{grid-template-columns:38px 38px minmax(0,1fr)}.row-value{grid-column:1/-1;text-align:left;padding-left:88px}.embedded-video-list.icon-anchor{grid-column:1/-1}.embedded-video-card{width:100%;max-width:none}}</style></head>
...
<aside class="side-stack"><section class="panel side-block"><h3 class="side-info-head">現在のランキング情報</h3><div class="side-info"><div class="side-info-row"><span>更新日</span><strong>__UPDATED_DATE__</strong></div><div class="side-info-row"><span>本日見つけた動画数</span><strong>__NEW_24H__</strong></div><div class="side-info-row"><span>24h再生増加数</span><strong class="metric-growth views"><span class="ico">▶️</span>+__TOTAL_GROWTH__</strong></div><div class="side-info-row"><span>総再生数</span><strong>__TOTAL_VIEWS__</strong></div><div class="side-info-row"><span>24h Like増加数</span><strong class="metric-growth likes"><span class="ico">❤</span>+__LIKES_24H__</strong></div><div class="side-info-row"><span>総like数</span><strong>__TOTAL_LIKES__</strong></div><div class="side-info-row"><span>24h コメント増加数</span><strong class="metric-growth comments"><span class="ico">💬</span>+__COMMENTS_24H__</strong></div><div class="side-info-row"><span>総コメント数</span><strong>__TOTAL_COMMENTS__</strong></div></div></section><section class="panel"><h2 class="pickup-head">✨️新着ピックアップ</h2><div class="pickup-list">__PICKUP_HTML__</div></section></aside></section></main>
<footer class="footer"><div class="footer-links"><a href="/policy">プライバシーポリシー</a><a href="https://x.com/Vcliprank" target="_blank" rel="noopener noreferrer">お問い合わせ</a></div><span>VCLIP | VTuber切り抜きランキング &copy; 2026</span></footer><script>const tabs=document.querySelectorAll(".tab");const panels=document.querySelectorAll(".panel-view");tabs.forEach((tab)=>tab.addEventListener("click",()=>{tabs.forEach((t)=>t.classList.remove("active"));panels.forEach((p)=>p.classList.remove("active"));tab.classList.add("active");const target=document.getElementById(tab.dataset.target||"");if(target)target.classList.add("active");}));</script></body></html>"""
    return (
        ...
        .replace("__UPDATED_DATE__", html.escape(updated_date))
        ...
        .replace("__NEW_24H__", f"{int(hero_stats.get('new_24h') or 0):,}")
        .replace("__TRACKING_VIDEOS__", f"{int(hero_stats.get('tracking_videos') or 0):,}")
        .replace("__TOTAL_VIEWS__", f"{int(side_metrics.get('total_views') or 0):,}")
        .replace("__LIKES_24H__", f"{int(side_metrics.get('likes_24h') or 0):,}")
        .replace("__TOTAL_LIKES__", f"{int(side_metrics.get('total_likes') or 0):,}")
        .replace("__COMMENTS_24H__", f"{int(side_metrics.get('comments_24h') or 0):,}")
        .replace("__TOTAL_COMMENTS__", f"{int(side_metrics.get('total_comments') or 0):,}")
        .replace("__TOP_GROWTH_NAME__", top_growth_name)
        .replace("__TOP_GROWTH_VALUE__", f"{top_growth_value:,}")
        .replace("__TOP_RATE_NAME__", top_rate_name)
        .replace("__TOP_RATE_VALUE__", f"{top_rate_value:.1f}")
...
<body class="{body_class}" id="top">
  ...
  <script>
    ...
    async function fetchPeriodPayload(periodTable) {{
      ...
      for (const url of candidates) {{
        try {{
          ...
          if (!data || !data.ok || !data.period || data.period.table !== periodTable) continue;
          return data.period;
        }} catch (_error) {{
          continue;
        }}
    ...
    async function ensurePeriodLoaded(periodTable) {{
      const period = periodMap.get(periodTable);
      if (!period || !period.lazy) return true;
      if (loadingPeriods.has(periodTable)) {{
        return loadingPeriods.get(periodTable);
      }}
      const loadPromise = (async () => {{
        const loaded = await fetchPeriodPayload(periodTable);
        if (!loaded) {{
          const fallback = periodMap.get(periodTable);
          if (fallback) {{
            fallback.lazy = false;
            fallback.groups = {{
              all: '<div class="empty">この期間のランキング取得に失敗しました。時間をおいて再試行してください。</div>',
            }};
        ...
        const target = periodMap.get(periodTable);
        if (target) {{
          Object.keys(target).forEach((key) => delete target[key]);
          Object.assign(target, loaded);
        }} else {{
          periodMap.set(periodTable, loaded);
        }}
        builtPeriodPanels.delete(periodTable);
        return true;
      }})();
      loadingPeriods.set(periodTable, loadPromise);
      try {{
        return await loadPromise;
      }} finally {{
        loadingPeriods.delete(periodTable);
      }}
    }}
...
def _fetch_video_detail_payload(video_id: str, period_key: str = "daily") -> dict | None:
    ...
    unique_related_rows: list[dict] = []
    seen_related_ranks: set[int] = set()
    for row in related_rows:
        try:
            best_rank_value = int(row.get("best_rank") or 0)
        except (TypeError, ValueError):
            best_rank_value = 0
        if best_rank_value <= 0 or best_rank_value in seen_related_ranks:
            continue
        seen_related_ranks.add(best_rank_value)
        unique_related_rows.append(row)
    related_rows = unique_related_rows[:3]
