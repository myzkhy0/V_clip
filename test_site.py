...
def render_channel_growth_page(base_url: str = "") -> str:
    ...
    for idx, item in enumerate(growth_ranked[:50], start=1):
        ...
        row_videos = f'<div class="embedded-video-list">{embedded_videos}</div>' if idx <= 10 else ""
        growth_items.append(
            ...
            f'<span class="row-meta">{html.escape(GROUP_LABELS.get(item.get("group_name") or "other", "その他"))} / 対象動画 {int(item.get("videos_with_base") or 0)}本</span>{row_videos}</span>'
            f'<span class="row-value">+{growth_value:,}</span></div>'
        )
    rate_items: list[str] = []
    for idx, item in enumerate(rate_ranked[:50], start=1):
        ...
        rate_items.append(
            ...
            f'<span class="avatar">{avatar}</span>'
            f'<span class="row-main"><a class="row-title-link" href="{link}" target="_blank" rel="noopener noreferrer">{channel_name}</a>'
            f'<span class="row-meta">{html.escape(GROUP_LABELS.get(item.get("group_name") or "other", "その他"))} / 24h前 {int(item.get("base_views_24h_ago") or 0):,}再生</span>{row_videos}</span>'
            f'<span class="row-value rate">+{rate_value:.1f}%</span></div>'
        )
...
<aside class="side-stack"><section class="panel side-block"><h3 class="side-info-head">現在のランキング情報</h3><div class="side-info"><div class="side-info-row"><span>更新日</span><strong>__UPDATED_DATE__</strong></div><div class="side-info-row"><span>本日見つけた動画数</span><strong>__NEW_24H__</strong></div><div class="side-info-row"><span>▶️24h再生増加数</span><strong class="metric-growth views">+__TOTAL_GROWTH__</strong></div><div class="side-info-row"><span>総再生増加数</span><strong>__TOTAL_GROWTH__</strong></div><div class="side-info-row"><span>❤24h Like増加数</span><strong class="metric-growth likes">+__LIKES_24H__</strong></div><div class="side-info-row"><span>総like数</span><strong>__TOTAL_LIKES__</strong></div><div class="side-info-row"><span>💬24h コメント増加数</span><strong class="metric-growth comments">+__COMMENTS_24H__</strong></div><div class="side-info-row"><span>総コメント数</span><strong>__TOTAL_COMMENTS__</strong></div></div></section><section class="panel"><h2 class="pickup-head">✨️新着ピックアップ</h2><div class="pickup-list">__PICKUP_HTML__</div></section></aside></section></main>
