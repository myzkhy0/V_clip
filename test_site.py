...
def render_channel_growth_page(base_url: str = "") -> str:
    ...
    for idx, item in enumerate(growth_ranked[:100], start=1):
    ...
    rate_items: list[str] = []
    for idx, item in enumerate(rate_ranked[:100], start=1):
        cls = "gold" if idx == 1 else ("silver" if idx == 2 else ("bronze" if idx == 3 else ""))
        channel_name = html.escape(item.get("channel_name") or "")
        channel_id = str(item.get("channel_id") or "")
        link = html.escape(f"https://www.youtube.com/channel/{quote(channel_id, safe='')}", quote=True) if channel_id else "#"
...
<style>:root{--bg:#fff;--header-bg:rgba(11,17,28,.92);--panel:#fff;--panel-border:#d3dde8;--text-main:#111827;--text-sub:#475569;--text-soft:#64748b;--text-primary:#e8edf4;--ok:#10b981;--header-border:rgba(99,177,230,.22);--shadow-sm:0 8px 18px rgba(15,23,42,.10);--shadow-md:0 14px 30px rgba(15,23,42,.16);--radius-md:12px}*{box-sizing:border-box}body{margin:0;font-family:"Noto Sans JP Local","Hiragino Kaku Gothic ProN",sans-serif;color:var(--text-main);background:radial-gradient(900px 420px at 10% -10%,rgba(99,208,255,.08),transparent 60%),radial-gradient(760px 380px at 92% -12%,rgba(96,165,250,.06),transparent 58%),var(--bg)}.header{position:sticky;top:0;z-index:100;background:var(--header-bg);border-bottom:1px solid rgba(100,160,240,.18);border-top:2px solid rgba(99,208,255,.34)}.header-inner{height:60px;padding:0 24px;display:flex;align-items:center;justify-content:space-between}.brand{display:flex;align-items:center;gap:12px;text-decoration:none;color:var(--text-primary)}.brand-logo{display:inline-flex;align-items:center;justify-content:center;width:44px;height:30px;border-radius:8px;background:linear-gradient(135deg,#63d0ff,#a78bfa);color:#fff;font-weight:900;font-size:.72rem}.brand-accent{background:linear-gradient(135deg,#63d0ff,#a78bfa);-webkit-background-clip:text;-webkit-text-fill-color:transparent}.main{max-width:1180px;margin:0 auto;padding:22px 24px 48px;display:grid;gap:14px}.panel{background:var(--panel);border:1px solid var(--panel-border);border-radius:var(--radius-md);box-shadow:var(--shadow-sm);padding:14px}.hero h1{margin:0;font-size:1.1rem;color:#0f294f}.hero p{margin:4px 0 0;color:var(--text-sub);font-size:.86rem}.content{display:grid;grid-template-columns:minmax(0,2fr) minmax(320px,1fr);gap:14px;align-items:start}.rank-section{margin-top:10px}.rank-section h2{margin:0 0 8px;font-size:1rem;color:#1f3344}.rank-list{display:grid;gap:10px}.rank-row{display:grid;grid-template-columns:42px 42px minmax(0,1fr) auto;gap:10px;align-items:center;border:1px solid var(--panel-border);border-radius:10px;background:#fff;padding:10px;text-decoration:none;color:inherit;transition:transform .2s,box-shadow .2s}.rank-row:hover{transform:translateY(-2px);box-shadow:var(--shadow-md)}.rank-badge{min-width:34px;height:34px;border-radius:7px;display:inline-flex;align-items:center;justify-content:center;font-size:.9rem;font-weight:800;color:#fff;background:rgba(26,32,44,.82)}.rank-badge.gold{background:linear-gradient(135deg,#f59e0b,#facc15);color:#3b2a00}.rank-badge.silver{background:linear-gradient(135deg,#94a3b8,#e2e8f0);color:#1f2937}.rank-badge.bronze{background:linear-gradient(135deg,#b45309,#d6a77a);color:#fff8ef}.avatar{width:40px;height:40px;border-radius:50%;overflow:hidden;background:#e7eef7;border:1px solid #d6e0ea;display:inline-flex;align-items:center;justify-content:center}.avatar img{width:100%;height:100%;object-fit:cover;display:block}.avatar-fallback{font-size:.72rem;font-weight:800;color:#33516f}.row-main{min-width:0;display:grid}.row-title{font-size:.94rem;color:#0f172a;font-weight:700;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.row-meta{color:var(--text-soft);font-size:.78rem}.row-value{text-align:right;font-weight:800;color:var(--ok);font-size:.95rem}.row-value.rate{color:#0369a1}.info-list{display:grid;gap:10px}.info-card{border:1px solid var(--panel-border);border-radius:10px;padding:10px;background:#f8fbff}.info-card .label{display:block;color:#47637e;font-size:.75rem;font-weight:700}.info-card .value{display:block;color:#0f172a;font-size:.95rem;font-weight:800;margin-top:2px}.info-card .value.growth{color:var(--ok)}.footer{width:100%;margin:0;padding:18px 24px 22px;text-align:center;font-size:.76rem;color:#dbeafe;border-top:1px solid var(--header-border);background:var(--header-bg)}.footer-links{display:flex;justify-content:center;gap:14px;margin-bottom:8px;font-size:.82rem}.footer-links a{color:#cfe0ff;text-decoration:none}@media (max-width:980px){.content{grid-template-columns:1fr}}@media (max-width:760px){.header-inner{padding:0 14px;height:54px}.main{padding:16px 14px 34px}.rank-row{grid-template-columns:38px 38px minmax(0,1fr)}.row-value{grid-column:1/-1;text-align:left;padding-left:88px}}</style></head>
<body><header class="header"><div class="header-inner"><a class="brand" href="/"><span class="brand-logo">VCLIP</span><span>VTuber切り抜き<span class="brand-accent">ランキング</span></span></a><div style="color:rgba(232,237,244,.90);font-size:.82rem;">最終更新: __UPDATED__</div></div></header>
<main class="main"><section class="panel hero"><h1>チャンネル24h急上昇ランキング</h1><p>24時間で伸びたチャンネルを「増加数」「増加率」で同時に確認できます。</p></section>
<section class="content"><article class="panel"><section class="rank-section"><h2>24h増加数ランキング</h2><div class="rank-list">__GROWTH_HTML__</div></section><section class="rank-section"><h2>24h増加率ランキング</h2><div class="rank-list">__RATE_HTML__</div></section></article>
<aside class="panel"><h2 style="margin:0 0 10px;color:#1f3344;font-size:1rem;font-weight:800;">ページ情報</h2><div class="info-list"><div class="info-card"><span class="label">更新基準</span><span class="value">各動画の最新計測値から24時間前との差分</span></div><div class="info-card"><span class="label">増加率ランキングの条件</span><span class="value">対象動画2本以上・24h前の合計再生1万以上</span></div><div class="info-card"><span class="label">24h総増加</span><span class="value growth">▶ +__TOTAL_GROWTH__</span></div><div class="info-card"><span class="label">集計チャンネル数（増加数）</span><span class="value">__GROWTH_COUNT__ チャンネル</span></div><div class="info-card"><span class="label">集計チャンネル数（増加率）</span><span class="value">__RATE_COUNT__ チャンネル</span></div></div></aside></section></main>
<footer class="footer"><div class="footer-links"><a href="/policy">プライバシーポリシー</a><a href="https://x.com/Vcliprank" target="_blank" rel="noopener noreferrer">お問い合わせ</a></div><span>VCLIP | VTuber切り抜きランキング &copy; 2026</span></footer></body></html>"""
    return (
        html_template
