...
def render_channel_growth_page(base_url: str = "") -> str:
    ...
    for idx, item in enumerate(growth_ranked[:50], start=1):
        ...
        top_video_ids = item.get("top_video_ids") or []
        top_video_links = "".join(
            f'<a class="mini-video-link" href="/video/{quote(str(video_id), safe="")}">動画{video_idx}</a>'
            for video_idx, video_id in enumerate(top_video_ids, start=1)
        )
        ...
        if idx <= 10:
            growth_top10_cards.append(
                ...
                f'<span class="card-main"><a class="row-title-link" href="{html.escape(detail_link, quote=True)}">{channel_name}</a><span class="row-meta">+{growth_value:,}</span><span class="mini-video-links">{top_video_links}</span></span></div>'
            )
        growth_items.append(
    ...
    for idx, item in enumerate(rate_ranked[:50], start=1):
        ...
        if idx <= 10:
            rate_top10_cards.append(
                f'<div class="top10-card">'
                f'<span class="rank-badge {cls}">{idx}</span><span class="avatar">{avatar}</span>'
                f'<span class="card-main"><a class="row-title-link" href="{html.escape(detail_link, quote=True)}">{channel_name}</a><span class="row-meta">+{rate_value:.1f}%</span><span class="mini-video-links">{top_video_links}</span></span></div>'
            )
        rate_items.append(
            f'<a class="rank-row" href="{link}" target="_blank" rel="noopener noreferrer">'
            f'<span class="rank-badge {cls}">{idx}</span>'
