    ...
    for idx, item in enumerate(growth_ranked[:50], start=1):
        ...
        if idx <= 10:
            growth_top10_cards.append(
                f'<div class="top10-card">'
                f'<span class="rank-badge {cls}">{idx}</span><span class="avatar">{avatar}</span>'
                f'<span class="card-main"><a class="row-title-link" href="{html.escape(detail_link, quote=True)}">{channel_name}</a><span class="row-meta">+{growth_value:,}</span><span class="embedded-video-list">{embedded_videos}</span></span></div>'
            )
        growth_items.append(
            ...
            f'<span class="row-value">+{growth_value:,}</span></a>'
        )
    rate_top10_cards: list[str] = []
    rate_items: list[str] = []
    for idx, item in enumerate(rate_ranked[:50], start=1):
        ...
        if idx <= 10:
            rate_top10_cards.append(
                f'<div class="top10-card">'
                f'<span class="rank-badge {cls}">{idx}</span><span class="avatar">{avatar}</span>'
                f'<span class="card-main"><a class="row-title-link" href="{html.escape(detail_link, quote=True)}">{channel_name}</a><span class="row-meta">+{rate_value:.1f}%</span><span class="embedded-video-list">{embedded_videos}</span></span></div>'
            )
        rate_items.append(
            f'<a class="rank-row" href="{link}" target="_blank" rel="noopener noreferrer">'
            f'<span class="rank-badge {cls}">{idx}</span>'
            ...
            f'<span class="row-value rate">+{rate_value:.1f}%</span></a>'
        )

    growth_top10_html = "".join(growth_top10_cards)
    rate_top10_html = "".join(rate_top10_cards)
...
<style>:root{--bg:#fff;--header-bg:rgba(11,17,28,.92);--panel:#fff;--panel-border:#d3dde8;--text-main:#111827;--text-sub:#475569;--text-soft:#64748b;--text-primary:#e8edf4;--ok:#10b981;--header-border:rgba(99,177,230,.22);--shadow-sm:0 8px 18px rgba(15,23,42,.10);--shadow-md:0 14px 30px rgba(15,23,42,.16);--radius-md:12px}*{box-sizing:border-box}body{margin:0;font-family:"Noto Sans JP Local","Hiragino Kaku Gothic ProN",sans-serif;color:var(--text-main);background:radial-gradient(900px 420px at 10% -10%,rgba(99,208,255,.08),transparent 60%),radial-gradient(760px 380px at 92% -12%,rgba(96,165,250,.06),transparent 58%),var(--bg)}.header{position:sticky;top:0;z-index:100;background:var(--header-bg);border-bottom:1px solid rgba(100,160,240,.18);border-top:2px solid rgba(99,208,255,.34)}.header-inner{height:60px;padding:0 24px;display:flex;align-items:center;justify-content:space-between}.brand{display:flex;align-items:center;gap:12px;text-decoration:none;color:var(--text-primary)}.brand-logo{display:inline-flex;align-items:center;justify-content:center;width:44px;height:30px;border-radius:8px;background:linear-gradient(135deg,#63d0ff,#a78bfa);color:#fff;font-weight:900;font-size:.72rem}.brand-text{font-weight:800;font-size:1.05rem}.brand-accent{background:linear-gradient(135deg,#63d0ff,#a78bfa);-webkit-background-clip:text;-webkit-text-fill-color:transparent}.header-meta{color:rgba(232,237,244,.90);font-size:.82rem}.live-dot{display:inline-flex;align-items:center;gap:6px;color:#10b981;font-weight:600}.live-dot::before{content:'';width:7px;height:7px;border-radius:50%;background:#10b981;animation:pulse-dot 2s ease-in-out infinite}@keyframes pulse-dot{0%,100%{opacity:1;transform:scale(1)}50%{opacity:.5;transform:scale(.8)}}.main{max-width:1180px;margin:0 auto;padding:22px 24px 48px;display:grid;gap:14px}.panel{background:var(--panel);border:1px solid var(--panel-border);border-radius:var(--radius-md);box-shadow:var(--shadow-sm);padding:14px}.hero h1{margin:0;font-size:1.1rem;color:#0f294f}.hero p{margin:4px 0 0;color:var(--text-sub);font-size:.86rem}.content{display:grid;grid-template-columns:minmax(0,2fr) minmax(320px,1fr);gap:14px;align-items:start}.tabs{display:inline-flex;border:1px solid var(--panel-border);border-radius:10px;overflow:hidden;margin-top:8px}.tab{border:0;background:transparent;color:#64748b;padding:6px 10px;font-size:.76rem;font-weight:700;cursor:pointer}.tab.active{color:#fff;background:linear-gradient(135deg,rgba(99,208,255,.72),rgba(96,165,250,.82))}.panel-view{display:none}.panel-view.active{display:block}.top10-grid{display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:10px}.top10-card{display:grid;grid-template-columns:34px 34px minmax(0,1fr);gap:8px;align-items:start;border:1px solid var(--panel-border);border-radius:9px;background:#f8fbff;padding:7px}.card-main{min-width:0;display:grid;gap:6px}.row-title-link{font-size:.94rem;color:#0f172a;font-weight:700;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;text-decoration:none}.row-title-link:hover{color:#075985;text-decoration:underline}.embedded-video-list{display:grid;gap:5px}.embedded-video-card{display:grid;grid-template-columns:56px minmax(0,1fr);gap:6px;align-items:center;padding:4px 6px;border:1px solid #c7d6e7;border-radius:8px;background:#fff;color:#334155;text-decoration:none}.embedded-video-card:hover{background:#f7fbff}.embedded-video-thumb{width:56px;height:32px;border-radius:6px;overflow:hidden;background:#111827;display:inline-flex;align-items:center;justify-content:center}.embedded-video-thumb img{width:100%;height:100%;object-fit:cover;display:block}.embedded-video-thumb-fallback{font-size:.56rem;color:#e2e8f0;font-weight:700}.embedded-video-title{font-size:.74rem;line-height:1.2;color:#334155;display:-webkit-box;-webkit-box-orient:vertical;-webkit-line-clamp:2;overflow:hidden}.rank-list{display:grid;gap:10px}.rank-row{display:grid;grid-template-columns:42px 42px minmax(0,1fr) auto;gap:10px;align-items:center;border:1px solid var(--panel-border);border-radius:10px;background:#fff;padding:10px;text-decoration:none;color:inherit;transition:transform .2s,box-shadow .2s}.rank-row:hover{transform:translateY(-2px);box-shadow:var(--shadow-md)}.rank-badge{min-width:34px;height:34px;border-radius:7px;display:inline-flex;align-items:center;justify-content:center;font-size:.9rem;font-weight:800;color:#fff;background:rgba(26,32,44,.82)}.rank-badge.gold{background:linear-gradient(135deg,#f59e0b,#facc15);color:#3b2a00}.rank-badge.silver{background:linear-gradient(135deg,#94a3b8,#e2e8f0);color:#1f2937}.rank-badge.bronze{background:linear-gradient(135deg,#b45309,#d6a77a);color:#fff8ef}.avatar{width:40px;height:40px;border-radius:50%;overflow:hidden;background:#e7eef7;border:1px solid #d6e0ea;display:inline-flex;align-items:center;justify-content:center}.top10-card .avatar{width:34px;height:34px}.avatar img{width:100%;height:100%;object-fit:cover;display:block}.avatar-fallback{font-size:.72rem;font-weight:800;color:#33516f}.row-main{min-width:0;display:grid}.row-title{font-size:.94rem;color:#0f172a;font-weight:700;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.row-meta{color:var(--text-soft);font-size:.78rem}.row-value{text-align:right;font-weight:800;color:var(--ok);font-size:.95rem}.row-value.rate{color:#0369a1}.info-list{display:grid;gap:8px}.info-row{display:flex;justify-content:space-between;gap:8px;font-size:.84rem;color:var(--text-sub)}.info-row strong{color:#0f172a}.info-row .growth{color:var(--ok);font-weight:800}.footer{width:100%;margin:0;padding:18px 24px 22px;text-align:center;font-size:.76rem;color:#dbeafe;border-top:1px solid var(--header-border);background:var(--header-bg)}.footer-links{display:flex;justify-content:center;gap:14px;margin-bottom:8px;font-size:.82rem}.footer-links a{color:#cfe0ff;text-decoration:none}@media (max-width:980px){.content{grid-template-columns:1fr}}@media (max-width:760px){.header-inner{padding:0 14px;height:54px}.header-meta{display:none}.main{padding:16px 14px 34px}.top10-grid{grid-template-columns:1fr}.rank-row{grid-template-columns:38px 38px minmax(0,1fr)}.row-value{grid-column:1/-1;text-align:left;padding-left:88px}}</style></head>
...
<section class="content"><article class="panel"><div id="growth" class="panel-view active"><div class="top10-grid">__GROWTH_TOP10_HTML__</div><div class="rank-list">__GROWTH_HTML__</div></div><div id="rate" class="panel-view"><div class="top10-grid">__RATE_TOP10_HTML__</div><div class="rank-list">__RATE_HTML__</div></div></article>
...
<footer class="footer"><div class="footer-links"><a href="/policy">プライバシーポリシー</a><a href="https://x.com/Vcliprank" target="_blank" rel="noopener noreferrer">お問い合わせ</a></div><span>VCLIP | VTuber切り抜きランキング &copy; 2026</span></footer><script>const tabs=document.querySelectorAll(".tab");const panels=document.querySelectorAll(".panel-view");tabs.forEach((tab)=>tab.addEventListener("click",()=>{tabs.forEach((t)=>t.classList.remove("active"));panels.forEach((p)=>p.classList.remove("active"));tab.classList.add("active");const target=document.getElementById(tab.dataset.target||"");if(target)target.classList.add("active");}));</script></body></html>"""
    return (
        ...
        .replace("__GROWTH_TOP10_HTML__", growth_top10_html)
        .replace("__RATE_TOP10_HTML__", rate_top10_html)
